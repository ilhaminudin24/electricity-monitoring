...
const HowItWorksEditor = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const content = await fetchCMSSection('how_it_works');

                if (content) {
                    setFormData({
                        ...content,
                        title: {
                            en: content.title?.en || 'How It Works',
                            id: content.title?.id || 'Cara Kerja'
                        },
                        subtitle: {
                            en: content.subtitle?.en || 'Simple steps to get started',
                            id: content.subtitle?.id || 'Langkah mudah untuk memulai'
                        },
                    ...
                    setFormData({
                        title: { en: 'How It Works', id: 'Cara Kerja' },
                        subtitle: { en: 'Simple steps to get started', id: 'Langkah mudah untuk memulai' },
                        items: []
                    });
                }
            } catch (error) {
    ...
    const handleSave = async () => {
        try {
            ...
            await saveCMSSection('how_it_works', dataToSave, currentUser?.id);

            setFormData(dataToSave);
            setMessage({
                type: 'success',
                text: 'How It Works saved and published successfully!'
            });
    ...
    return (
        <div className="p-8 max-w-6xl mx-auto">
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900">How It Works Section</h1>
                    <p className="text-slate-600 mt-1">Manage the steps guide</p>
                </div>
            ...
            <div className="space-y-8">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                    <ReorderableList
                        title="Steps"
                        description="Add, remove, and reorder steps"
                        ...
                        renderItem={(item, index) => (
                            <div className="space-y-4">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-slate-500 text-sm font-medium">Step {index + 1}</span>
                                </div>
                                <DualLanguageInput
                                    label="Step Title"
                                    valueEN={item.title?.en || ''}
                                    valueID={item.title?.id || ''}
                                    onChange={(lang, value) => handleStepLangChange(index, lang, 'title', value)}
                                />
                                <DualLanguageInput
                                    label="Step Description"
                                    valueEN={item.description?.en || ''}
                                    valueID={item.description?.id || ''}
                                    onChange={(lang, value) => handleStepLangChange(index, lang, 'description', value)}
                                    multiline
                                />
                            </div>
                        )}
